...
class HistoryController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = [
            'js'        => 'components.scripts.BE.admin.manage.history',
            'title'     => 'history',
            'setting'    => $setting,
        ];

        return view('pages.BE.admin.manage.history', $data);
    }
    // NOTE GET /manage/refferal
        ...
        try {
            if (is_numeric($any)) {
                $data = DB::table('users')
                    ...
                    ->select([
                        'users.*', 'orders.biaya_adm','orders.total','orders.sub_total',
            ...
            $data = DB::table('users')
                    ...
                    ->select([
                        'users.*', 'orders.biaya_adm','orders.total','orders.sub_total',
                        'orders.status','orders.created_at as waktu'
                    ])
                ->orderBy('name', 'asc');
            ...
            return DataTables::of($data)
                ...
                ->addColumn(
                    ...
                    function ($row) {
                        ...
                        return view('components.buttons.BE.manage.history', $data);
                    }
                )
                ->addIndexColumn()
                ->make(true);
